...
function fail(code){
  console.log(`physical-computer-use-perception-p9b-s14=FAIL code=${code}`);
  process.exit(1);
}

if(process.platform!=="darwin"){
  console.log("physical-computer-use-perception-p9b-s14=BLOCKED code=MACOS_REQUIRED");
  process.exit(2);
}
...
const refreshedSemanticCleanup='const cleanupRuntime=computerControl.ensureRuntime();const cleanupForeground=cleanupRuntime?.ok===true?computerControl.activateApplication({app:"Pulsar",timeoutMs:5000}):{ok:false,state:"SKIPPED",error:"RUNTIME_REOPEN_FAILED"};const dismissed=cleanupForeground?.ok===true?computerControl.press({app:"Pulsar",keys:"Escape",settle:false}):{ok:false,state:"SKIPPED",error:cleanupRuntime?.ok===true?"FOREGROUND_REASSERT_FAILED":"RUNTIME_REOPEN_FAILED"};selectorDismissed=dismissed?.ok!==false;';
if(source.split(semanticCleanup).length-1!==1)fail("SELECTOR_CLEANUP_PATCH_ANCHOR_INVALID");
source=source.replace(semanticCleanup,refreshedSemanticCleanup);

const tmp=fs.mkdtempSync(path.join(os.tmpdir(),"rumiai-p9b-s14-"));
const patched=path.join(tmp,"p9b-s14-physical.js");
...
try{
  ...
  if((child.status??1)===0){
    console.log("p9b-s14-fixture-correction=PASS baseBlobVerified=true generatedPreloadNewlineEscape=true distinctComputerControlSockets=true parentRuntimeReensuredAfterChildCleanup=true cleanupForegroundReasserted=true cleanupUsesSemanticUiPressEscape=true productChanged=false");
    console.log("physical-computer-use-perception-p9b-s14=PASS");
    process.exit(0);
  }
  if((child.status??1)===2){
    console.log("physical-computer-use-perception-p9b-s14=BLOCKED code=PATCHED_PHYSICAL_BLOCKED");
    process.exit(2);
  }
  console.log(`physical-computer-use-perception-p9b-s14=FAIL code=PATCHED_PHYSICAL_FAILED childStatus=${child.status??"null"} childSignal=${child.signal??"none"}`);
  process.exit(1);
}finally{

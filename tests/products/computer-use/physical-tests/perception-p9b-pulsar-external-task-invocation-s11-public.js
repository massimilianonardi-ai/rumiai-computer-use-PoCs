...
function fail(code){
  console.log(`physical-computer-use-perception-p9b-s11=FAIL code=${code}`);
  process.exit(1);
}

if(process.platform!=="darwin"){
  console.log("physical-computer-use-perception-p9b-s11=BLOCKED code=MACOS_REQUIRED");
  process.exit(2);
}
...
try{
  ...
  if((child.status??1)===0){
    console.log("p9b-s11-fixture-correction=PASS baseBlobVerified=true generatedPreloadNewlineEscape=true distinctComputerControlSockets=true cleanupForegroundReasserted=true cleanupUsesSemanticUiPressEscape=true productChanged=false");
    console.log("physical-computer-use-perception-p9b-s11=PASS");
    process.exit(0);
  }
  if((child.status??1)===2){
    console.log("physical-computer-use-perception-p9b-s11=BLOCKED code=PATCHED_PHYSICAL_BLOCKED");
    process.exit(2);
  }
  console.log(`physical-computer-use-perception-p9b-s11=FAIL code=PATCHED_PHYSICAL_FAILED childStatus=${child.status??"null"} childSignal=${child.signal??"none"}`);
  process.exit(1);
}finally{

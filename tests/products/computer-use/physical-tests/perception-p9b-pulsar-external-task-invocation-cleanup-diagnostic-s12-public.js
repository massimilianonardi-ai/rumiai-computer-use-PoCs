...
function fail(code){
  console.log(`physical-computer-use-perception-p9b-s12=FAIL code=${code}`);
  process.exit(1);
}

if(process.platform!=="darwin"){
  console.log("physical-computer-use-perception-p9b-s12=BLOCKED code=MACOS_REQUIRED");
  process.exit(2);
}
...
try{
  ...
  if((child.status??1)===2){
    console.log("physical-computer-use-perception-p9b-s12=BLOCKED code=PATCHED_PHYSICAL_BLOCKED");
    process.exit(2);
  }

  const detailObserved=stdout.includes("p9b-s12-cleanup-detail=OBSERVED");
  const coreVerified=
  ...
  console.log(`p9b-s12-diagnostic=PASS classification=SEMANTIC_CLEANUP_DETAIL_OBSERVED childStatus=${child.status??"null"} childSignal=${child.signal??"none"} corePathVerified=true productChanged=false`);
  console.log("physical-computer-use-perception-p9b-s12=PASS");
  process.exit(0);
}finally{

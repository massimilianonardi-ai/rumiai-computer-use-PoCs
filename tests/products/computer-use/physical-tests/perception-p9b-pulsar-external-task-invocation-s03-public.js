...
function fail(code){
  console.log(`physical-computer-use-perception-p9b-s03=FAIL code=${code}`);
  process.exit(1);
}

if(process.platform!=="darwin"){
  console.log("physical-computer-use-perception-p9b-s03=BLOCKED code=MACOS_REQUIRED");
  process.exit(2);
}
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),"rumiai-p9b-s03-"));
const patched=path.join(tmp,"p9b-s03-physical.js");
...
try{
  ...
  if((child.status??1)===0){
    console.log("p9b-s03-fixture-correction=PASS baseBlobVerified=true correction=generated-preload-newline-escape-only productChanged=false");
    console.log("physical-computer-use-perception-p9b-s03=PASS");
    process.exit(0);
  }
  if((child.status??1)===2){
    console.log("physical-computer-use-perception-p9b-s03=BLOCKED code=PATCHED_PHYSICAL_BLOCKED");
    process.exit(2);
  }
  console.log(`physical-computer-use-perception-p9b-s03=FAIL code=PATCHED_PHYSICAL_FAILED childStatus=${child.status??"null"}`);
  process.exit(1);
}finally{

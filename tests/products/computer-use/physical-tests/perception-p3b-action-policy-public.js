...
if(!productRoot){console.error("physical-computer-use-perception-p3b=BLOCKED missing RUMIAI_COMPUTER_USE_ROOT");process.exit(2);}
...
const policyGate=require(path.join(productRoot,"app","perception-action-policy.js"));
...
(async()=>{
  ...
  let outcome={code:1,marker:"physical-computer-use-perception-p3b=FAIL code=UNEXPECTED"};
  try{
    tmp=fs.mkdtempSync(path.join(os.tmpdir(),"rumiai-p3b-"));
    ...
    if(!mapped?.ok||mapped.state!=="VISUAL_FRAME_MAPPED"){
      if(mapped?.state==="BLOCKED"){outcome={code:2,marker:`physical-computer-use-perception-p3b=BLOCKED code=${mapped.error||"CAPTURE_BLOCKED"}`};return;}
      fail(mapped?.error||"MAPPED_FRAME_ACQUISITION_FAILED");
    }

    const provider={
      id:"poc.macos-vision-text-region",locality:"local",capabilities:["text-region"],
      observe:frame=>{
        const ocr=spawnSync(ocrBin,[],{input:Buffer.from(frame.dataBase64,"base64"),encoding:"utf8",maxBuffer:16*1024*1024});
        ...
        return {state:"OBSERVED",coordinateSpace:{kind:"capture-pixel",origin:"top-left",width:Number(result.width),height:Number(result.height)},observations:result.items.map(item=>({kind:"text-region",text:item.text,confidence:Number(item.confidence),region:{x:Number(item.box.x),y:Number(item.box.y),width:Number(item.box.width),height:Number(item.box.height),coordinateSpace:{kind:"capture-pixel",origin:"top-left"}}}))};
      },
    };
    ...
    const decision=policyGate.evaluateVisualFallbackPolicy(
      resolved,
      {kind:"pointer-click",button:"left",display:"primary"},
      {allowVisualFallback:true}
    );
    if(!decision?.ok||decision.state!=="VISUAL_FALLBACK_AUTHORIZED")fail(decision?.error||"P3B_POLICY_AUTHORIZATION_FAILED");
    if(decision.actionPolicy?.state!=="AUTHORIZED"||decision.actionPlan?.state!=="READY")fail("P3B_ACTION_PLAN_INVALID");
    if(decision.actionPlan?.kind!=="pointer-click"||decision.actionPlan?.button!=="left"||decision.actionPlan?.display!=="primary")fail("P3B_ACTION_SHAPE_INVALID");
    if(decision.actionPlan?.point?.coordinateSpace?.kind!=="primary-display-logical"||decision.actionPlan?.point?.coordinateSpace?.origin!=="top-left")fail("P3B_ACTION_POINT_SPACE_INVALID");
    const expectedTarget=ready.targets.find(v=>v.id==="beta");
    if(!expectedTarget||!inside(decision.actionPlan.point,expectedTarget))fail("P3B_ACTION_PLAN_ORACLE_MISMATCH");
    if(decision.delivery?.state!=="NOT_ATTEMPTED")fail("P3B_DELIVERY_PREMATURE");
    if(decision.semanticConsequence?.state!=="NOT_OBSERVED")fail("P3B_CONSEQUENCE_PREMATURE");
    if(decision.semanticTarget?.semanticIdentity!==null||decision.semanticTarget?.actionable!==false)fail("P3B_TARGET_BOUNDARY_VIOLATED");

    const denied=policyGate.evaluateVisualFallbackPolicy(resolved,{kind:"pointer-click",button:"left",display:"primary"},{allowVisualFallback:false});
    if(denied?.state!=="VISUAL_FALLBACK_REJECTED"||denied.actionPolicy?.state!=="REJECTED"||denied.actionPlan?.state!=="NOT_CREATED")fail("P3B_EXPLICIT_DENIAL_FAILED");

    console.log("p3b-action-policy=PASS authorized=true explicitConsent=true actionPlan=READY");
    console.log("p3b-action-plan-oracle=PASS logicalPointInsideExpectedWindow=true coordinatesLogged=false");
    console.log("p3b-denial-path=PASS explicitConsent=false actionPlan=NOT_CREATED");
    console.log("p3b-execution-boundary=PASS delivery=NOT_ATTEMPTED semanticConsequence=NOT_OBSERVED pointerActionsPerformed=false keyboardActionsPerformed=false");
    console.log("p3b-semantic-boundary=PASS semanticIdentity=null targetIntrinsicActionable=false");
    console.log("p3b-provider-chain=PASS p2bProviderNeutral=true provider=macos-vision textLogged=false");
    console.log("p3b-payload-policy=PASS persisted=false payloadLogged=false ocrTextLogged=false coordinatesLogged=false");
    outcome={code:0,marker:"physical-computer-use-perception-p3b=PASS"};
  }catch(error){outcome={code:1,marker:`physical-computer-use-perception-p3b=FAIL code=${error.code||error.message||"UNEXPECTED"}`};}
  finally{
    ...
    console.log(`p3b-runtime-cleanup=${runtimeCleanup?"PASS":"FAIL"}`);
    if(!runtimeCleanup&&outcome.code===0)outcome={code:1,marker:"physical-computer-use-perception-p3b=FAIL code=RUNTIME_CLEANUP_FAILED"};

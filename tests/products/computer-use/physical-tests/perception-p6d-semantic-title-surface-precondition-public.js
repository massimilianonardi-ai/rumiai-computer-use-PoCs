...
async function waitForSemanticSurface(computerControl,surface,expected,rejected){
  let snapshotResult=null,expectedResult=null,rejectedResult=null,attempts=0;
  for(let i=0;i<40;i++){
    attempts=i+1;
    snapshotResult=computerControl.snapshot({app:"Safari",compact:true,settle:false});
    if(snapshotResult?.ok&&snapshotResult.snapshot){
      expectedResult=surface.evaluateSemanticSurfacePrecondition(
        {kind:"semantic-text",match:"exact",text:expected},
        {state:{snapshot:snapshotResult.snapshot}}
      );
      rejectedResult=surface.evaluateSemanticSurfacePrecondition(
        {kind:"semantic-text",match:"exact",text:rejected},
        {state:{snapshot:snapshotResult.snapshot}}
      );
      if(expectedResult?.ok&&expectedResult.state==="SURFACE_PRECONDITION_VERIFIED"&&rejectedResult?.reason==="SURFACE_PRECONDITION_NOT_MET")break;
    }
    await asyncSleep(100);
  }
  return {snapshotResult,expectedResult,rejectedResult,attempts};
}
...
(async()=>{
  ...
  try{
    ...
    tmp=fs.mkdtempSync(path.join(os.tmpdir(),"rumiai-p6d-semantic-title-"));
    ...
    fs.writeFileSync(path.join(contractDir,"p6d-safari-canvas.json"),JSON.stringify({
      ...
      surfacePrecondition:{kind:"semantic-text",match:"exact",text:SURFACE_GOOD},
    ...
    if(!activated?.ok)fail("SAFARI_ACTIVATION_FAILED");
    let foreground=null;
    for(let i=0;i<30;i++){
    ...
    if(!isSafariForeground(foreground))fail("SAFARI_NOT_FOREGROUND");
    ...
    if(selected.contracts[0]?.surfacePrecondition?.kind!=="semantic-text"||selected.contracts[0]?.surfacePrecondition?.text!==SURFACE_GOOD)fail("SURFACE_PRECONDITION_NOT_MATERIALIZED");

    const badPreflight=await waitForSemanticSurface(computerControl,surface,SURFACE_BAD,SURFACE_GOOD);
    if(!badPreflight.expectedResult?.ok||badPreflight.expectedResult.state!=="SURFACE_PRECONDITION_VERIFIED")fail("BAD_SURFACE_NOT_CURRENT");
    if(badPreflight.expectedResult?.metadata?.matchCount!==1)fail("BAD_SURFACE_MATCH_COUNT_INVALID");
    if(badPreflight.rejectedResult?.reason!=="SURFACE_PRECONDITION_NOT_MET")fail("GOOD_SURFACE_UNEXPECTEDLY_PRESENT_BEFORE_RELOAD");

    let badSurfaceResult=null,badVerifyCalls=0,badProviderSelectCalls=0;
    ...
    if(badVerifyCalls!==1)fail("WRONG_SURFACE_PRECONDITION_VERIFY_COUNT_INVALID");
    ...
    const activatedAgain=computerControl.activateApplication({app:"Safari",timeoutMs:10000});
    if(!activatedAgain?.ok)fail("GOOD_SURFACE_ACTIVATION_FAILED");
    const goodPreflight=await waitForSemanticSurface(computerControl,surface,SURFACE_GOOD,SURFACE_BAD);
    if(!goodPreflight.expectedResult?.ok||goodPreflight.expectedResult.state!=="SURFACE_PRECONDITION_VERIFIED")fail("GOOD_SURFACE_NOT_CURRENT");
    if(goodPreflight.expectedResult?.metadata?.matchCount!==1)fail("GOOD_SURFACE_MATCH_COUNT_INVALID");
    if(goodPreflight.rejectedResult?.reason!=="SURFACE_PRECONDITION_NOT_MET")fail("BAD_SURFACE_STILL_CURRENT");

    let goodSurfaceResult=null,goodVerifyCalls=0,goodProviderSelectCalls=0;
    ...
    if(goodSurfaceResult?.state!=="SURFACE_PRECONDITION_VERIFIED"||goodSurfaceResult?.metadata?.matchCount!==1)fail("GOOD_SURFACE_PRECONDITION_NOT_VERIFIED");
    if(goodVerifyCalls!==1)fail("GOOD_SURFACE_PRECONDITION_VERIFY_COUNT_INVALID");
    if(goodProviderSelectCalls!==1)fail("GOOD_SURFACE_PROVIDER_SELECTION_COUNT_INVALID");
    if(openedVisual.visualFallbackProviderSelection?.surfacePrecondition?.state!=="VERIFIED"||openedVisual.visualFallbackProviderSelection?.surfacePrecondition?.kind!=="semantic-text")fail("GOOD_SURFACE_METADATA_MISSING");
    ...
    console.log(`p6d-real-surfaces=PASS application=Safari pageRequests=${pageRequests} modePolls=${modePolls} sameDocument=true sameVisualTarget=true surfaceIdentity=semantic-text-exact`);
    console.log(`p6d-negative-surface=PASS precondition=SURFACE_PRECONDITION_NOT_MET preflightMatchCount=1 verifierCalls=${badVerifyCalls} providerSelectionCalls=0 visualFallbackClickDeliveries=0 failClosed=true`);
    console.log(`p6d-surface-transition=PASS from=BETA to=ALPHA currentSurfaceFreshlyObserved=true observation=semantic-snapshot-exact sameTabReload=true browserTabAmbiguity=false matchCount=${goodPreflight.expectedResult.metadata.matchCount}`);
    console.log(`p6d-positive-surface=PASS precondition=SURFACE_PRECONDITION_VERIFIED verifierCalls=${goodVerifyCalls} providerSelectionCalls=1 executionPath=visual-fallback`);

...
function fail(code){
  console.log(`physical-computer-use-perception-p9b-s09=FAIL code=${code}`);
  process.exit(1);
}

if(process.platform!=="darwin"){
  console.log("physical-computer-use-perception-p9b-s09=BLOCKED code=MACOS_REQUIRED");
  process.exit(2);
}
...
const keyboardCleanup='const cleanupForeground=computerControl.activateApplication({app:"Pulsar",timeoutMs:5000});const dismissed=cleanupForeground?.ok===true?computerControl.pressKey({key:"Escape"}):{ok:false};selectorDismissed=dismissed?.ok!==false;';
const cleanupOccurrences=source.split(semanticCleanup).length-1;
if(cleanupOccurrences!==1)fail("SELECTOR_CLEANUP_PATCH_ANCHOR_INVALID");
source=source.replace(semanticCleanup,keyboardCleanup);

const tmp=fs.mkdtempSync(path.join(os.tmpdir(),"rumiai-p9b-s09-"));
const patched=path.join(tmp,"p9b-s09-physical.js");
...
try{
  ...
  if((child.status??1)===0){
    console.log("p9b-s09-fixture-correction=PASS baseBlobVerified=true generatedPreloadNewlineEscape=true distinctComputerControlSockets=true cleanupForegroundReasserted=true cleanupUsesPressKey=true productChanged=false");
    console.log("physical-computer-use-perception-p9b-s09=PASS");
    process.exit(0);
  }
  if((child.status??1)===2){
    console.log("physical-computer-use-perception-p9b-s09=BLOCKED code=PATCHED_PHYSICAL_BLOCKED");
    process.exit(2);
  }
  console.log(`physical-computer-use-perception-p9b-s09=FAIL code=PATCHED_PHYSICAL_FAILED childStatus=${child.status??"null"} childSignal=${child.signal??"none"}`);
  process.exit(1);
}finally{

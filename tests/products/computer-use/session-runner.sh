...
: "${EXPECTED_PRODUCT_SHA:?EXPECTED_PRODUCT_SHA required}"
: "${EXPECTED_CONTROL_SHA:?EXPECTED_CONTROL_SHA required}"
: "${TEST_SOURCE_SHA:?TEST_SOURCE_SHA required}"
: "${PHYSICAL_TEST:?PHYSICAL_TEST required}"
: "${CONTRACT_TESTS:?CONTRACT_TESTS required}"
PHYSICAL_TEST_ID="${PHYSICAL_TEST_ID:-physical:${SESSION_ID}}"
SESSION_REL="tests/products/computer-use/sessions/${SESSION_ID}"
...
TIMEOUT_RUNNER="$POC_ROOT/tests/products/computer-use/run-with-timeout.js"
PHYSICAL_TIMEOUT_MS="${PHYSICAL_TIMEOUT_MS:-45000}"
...
[ -f "$TIMEOUT_RUNNER" ] || fail_preflight "timeout runner missing: $TIMEOUT_RUNNER"
...
for test_file in $CONTRACT_TESTS; do
  [ -f "$POC_ROOT/$test_file" ] || fail_preflight "contract test missing: $test_file"
done
...
while IFS= read -r changed; do
  [ -z "$changed" ] && continue
  case "$changed" in "$SESSION_REL"/*) ;; *) fail_preflight "unexpected change after test-source checkpoint: $changed" ;; esac
...
git -C "$PRODUCT_ROOT" fetch origin main --quiet || fail_preflight "cannot fetch Computer Use origin/main"
PRODUCT_HEAD="$(git -C "$PRODUCT_ROOT" rev-parse HEAD)"
PRODUCT_REMOTE="$(git -C "$PRODUCT_ROOT" rev-parse origin/main)"
[ "$PRODUCT_HEAD" = "$EXPECTED_PRODUCT_SHA" ] || fail_preflight "Computer Use local HEAD $PRODUCT_HEAD != expected $EXPECTED_PRODUCT_SHA"
[ "$PRODUCT_REMOTE" = "$EXPECTED_PRODUCT_SHA" ] || fail_preflight "Computer Use origin/main $PRODUCT_REMOTE != expected $EXPECTED_PRODUCT_SHA"
[ -z "$(git -C "$PRODUCT_ROOT" status --porcelain)" ] || fail_preflight "Computer Use working tree must be clean before execution"

git -C "$CONTROL_ROOT" fetch origin main --quiet || fail_preflight "cannot fetch Computer Control origin/main"
CONTROL_HEAD="$(git -C "$CONTROL_ROOT" rev-parse HEAD)"
CONTROL_REMOTE="$(git -C "$CONTROL_ROOT" rev-parse origin/main)"
[ "$CONTROL_HEAD" = "$EXPECTED_CONTROL_SHA" ] || fail_preflight "Computer Control local HEAD $CONTROL_HEAD != expected $EXPECTED_CONTROL_SHA"
[ "$CONTROL_REMOTE" = "$EXPECTED_CONTROL_SHA" ] || fail_preflight "Computer Control origin/main $CONTROL_REMOTE != expected $EXPECTED_CONTROL_SHA"
[ -z "$(git -C "$CONTROL_ROOT" status --porcelain)" ] || fail_preflight "Computer Control working tree must be clean before execution"
...
{
  echo "RumiAI Computer Use physical session"
  echo "session_id=$SESSION_ID"
  echo "started_at=$STARTED_AT"
  echo "computer_use_root=$PRODUCT_ROOT"
  echo "computer_use_expected_sha=$EXPECTED_PRODUCT_SHA"
  echo "computer_use_observed_sha=$PRODUCT_HEAD"
  echo "computer_control_root=$CONTROL_ROOT"
  echo "computer_control_expected_sha=$EXPECTED_CONTROL_SHA"
  echo "computer_control_observed_sha=$CONTROL_HEAD"
  ...
  echo "physical_timeout_ms=$PHYSICAL_TIMEOUT_MS"
  echo
} > "$LOG_REL"
...
run_test "$PHYSICAL_TEST_ID" "AGENT_CTRL='$AGENT_CTRL' RUMIAI_COMPUTER_USE_ROOT='$PRODUCT_ROOT' RUMIAI_COMPUTER_CONTROL_HOME='$CONTROL_ROOT' RUMIAI_CC_NODE='$NODE' '$NODE' '$TIMEOUT_RUNNER' '$PHYSICAL_TIMEOUT_MS' '$NODE' '$POC_ROOT/$PHYSICAL_TEST'"
...
SESSION_ID="$SESSION_ID" STARTED_AT="$STARTED_AT" ENDED_AT="$ENDED_AT" TOTAL_MS="$TOTAL_MS" EXPECTED_PRODUCT_SHA="$EXPECTED_PRODUCT_SHA" PRODUCT_HEAD="$PRODUCT_HEAD" EXPECTED_CONTROL_SHA="$EXPECTED_CONTROL_SHA" CONTROL_HEAD="$CONTROL_HEAD" TEST_SOURCE_SHA="$TEST_SOURCE_SHA" POC_HEAD="$POC_HEAD" RESULTS_TSV="$RESULTS_TSV" RESULT_REL="$RESULT_REL" "$NODE" <<'NODE'
...
const summary={pass:rows.filter(x=>x.result==='PASS').length,fail:rows.filter(x=>x.result==='FAIL').length,blocked:rows.filter(x=>x.result==='BLOCKED').length,total:rows.length};summary.overall=summary.fail?'FAIL':summary.blocked?'BLOCKED':'PASS';
fs.writeFileSync(process.env.RESULT_REL,JSON.stringify({sessionId:process.env.SESSION_ID,startedAt:process.env.STARTED_AT,endedAt:process.env.ENDED_AT,durationMs:Number(process.env.TOTAL_MS),productShaExpected:process.env.EXPECTED_PRODUCT_SHA,productShaObserved:process.env.PRODUCT_HEAD,dependencies:{computerControl:{shaExpected:process.env.EXPECTED_CONTROL_SHA,shaObserved:process.env.CONTROL_HEAD}},testSourceSha:process.env.TEST_SOURCE_SHA,pocShaTested:process.env.POC_HEAD,tests:rows,summary},null,2)+'\n');
NODE
...
if ! git push origin main; then echo "EVIDENCE_PUSH=FAILED local_commit=$EVIDENCE_COMMIT" >&2; exit 4; fi

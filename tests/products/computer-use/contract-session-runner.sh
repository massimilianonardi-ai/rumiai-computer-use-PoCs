...
{
  echo "RumiAI Computer Use contract-only session"
  echo "session_id=$SESSION_ID"
  echo "validation_kind=contract-only"
...
fs.writeFileSync(process.env.RESULT_REL,JSON.stringify({sessionId:process.env.SESSION_ID,validationKind:'contract-only',startedAt:process.env.STARTED_AT,endedAt:process.env.ENDED_AT,durationMs:Number(process.env.TOTAL_MS),productShaExpected:process.env.EXPECTED_PRODUCT_SHA,productShaObserved:process.env.PRODUCT_HEAD,dependencies:{computerControl:{shaExpected:process.env.EXPECTED_CONTROL_SHA,shaObserved:process.env.CONTROL_HEAD}},testSourceSha:process.env.TEST_SOURCE_SHA,pocShaTested:process.env.POC_HEAD,tests:rows,summary},null,2)+'\n');
NODE

...
if /usr/bin/pgrep -x Pulsar >/dev/null 2>&1 || /usr/bin/pgrep -f '/Pulsar.app/Contents/MacOS/Pulsar' >/dev/null 2>&1; then
  ...
  echo "Pulsar is already running; close Pulsar before this diagnostic physical session so user editor state is not disturbed."
  exit 2
fi
export SESSION_ID="cu-perception-p9b-external-task-invocation-semantic-cleanup-diagnostic-public-s12"
...
export TEST_SOURCE_SHA="dbe4c26c698fcc9288c2991ca1592b8c0c4fcbc3"
...
export PHYSICAL_TEST="tests/products/computer-use/physical-tests/perception-p9b-pulsar-external-task-invocation-cleanup-diagnostic-s12-public.js"
export PHYSICAL_TEST_ID="physical:computer-use-perception-p9b-external-task-invocation-semantic-cleanup-diagnostic-s12-public"

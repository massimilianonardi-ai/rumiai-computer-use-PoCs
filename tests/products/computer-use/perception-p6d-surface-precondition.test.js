...
const rawContract={
  ...
  surfacePrecondition:{kind:"semantic-text",match:"exact",text:"P6D SURFACE ALPHA"},
...
test("P6D carries an exact semantic surface precondition without planner or delivery details",()=>{
...
test("P6D semantic surface precondition is exact, single-match and fail-closed",()=>{
  const precondition=rawContract.surfacePrecondition;
...
test("P6D checks the surface before perception-provider selection",()=>{
  const contract=executionContract();
  let selectCalls=0;
  const result=execution.resolveOpenVisualFallbackExecutionContext({
    intent:{intent:"OPEN",target:"PROCEED"},
    contract,
    runtimeContext:{state:{snapshot:'@e1 heading "P6D SURFACE BETA"\n'}},
  },{
...
test("P6D verified surface allows the existing lazy provider path",()=>{
  const contract=executionContract();
  let selectCalls=0;
  ...
  const result=execution.resolveOpenVisualFallbackExecutionContext({
    intent:{intent:"OPEN",target:"PROCEED"},
    contract,
    runtimeContext:{state:{snapshot:'@e1 heading "P6D SURFACE ALPHA"\n'}},
  },{
    selectProvider:()=>{
      selectCalls++;
      return {
...
test("P6D keeps surface identity outside planner and Computer Control",()=>{

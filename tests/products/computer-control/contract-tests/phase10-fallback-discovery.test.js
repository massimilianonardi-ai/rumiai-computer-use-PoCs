...
test("Phase 10 public low-level mechanics expose only evidence-backed closed fallbacks",()=>{
...
test("Phase 10A-10E mechanics are physically validated without weakening semantic APIs",()=>{
  ...
  assert.match(roadmap,/working semantic operation always takes precedence/);assert.match(roadmap,/coordinate delivery is not itself semantic success/);
  assert.match(phase10,/Phase 10A capture\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10B pointer\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10C drag\/drop\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10D wheel\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10E keyboard\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/low-level mechanics are complete/i);assert.match(phase10,/semantic text mutation/i);assert.match(phase10,/OCR\/vision.*separate interpretation layer/i);
});

...
test("Phase 10 public surface is evidence-backed through physically validated 10D before 10E discovery",()=>{
  ...
  for(const method of["display.capture","pointer.move","pointer.click","pointer.drag","pointer.wheel"])assert.equal(router.includes(`\"${method}\"`),true,method);
  for(const method of["pointer.down","pointer.up","pointer.button","input.scroll","input.wheel","wheel.post","keyboard.key","keyboard.type","keyboard.text","window.capture","ui.capture","ocr.read"])assert.equal(router.includes(`\"${method}\"`),false,method);
});

test("Phase 10 lifecycle validates through 10D and leaves 10E pending without weakening semantic APIs",()=>{
  ...
  assert.match(phase10,/Phase 10A capture\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10B pointer\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10C drag\/drop\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10D wheel\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/Phase 10E keyboard\s+PENDING/);assert.match(phase10,/semantic `ui\.scroll` remains preferred/i);
});

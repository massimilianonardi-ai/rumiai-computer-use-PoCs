...
test("Phase 10C docs preserve discovery evidence, fallback priority, release ownership and delivery-not-success",()=>{
  const api=fs.readFileSync(path.join(productRoot,"docs/api-pointer.md"),"utf8");const phase10=fs.readFileSync(path.join(productRoot,"docs/phase10-low-level-fallbacks.md"),"utf8");const evidence=fs.readFileSync(path.join(productRoot,"docs/evidence/phase10c-drag-delivery-discovery-physical.md"),"utf8");assert.match(api,/Phase 10C drag: `IMPLEMENTED`/);assert.match(api,/semanticConsequenceVerified.*false/);assert.match(api,/no public `pointer\.down` or `pointer\.up`/i);assert.match(phase10,/Phase 10C drag\/drop\s+IMPLEMENTED/);assert.match(phase10,/working semantic capability always takes precedence/i);assert.match(phase10,/result state is `DRAG_POSTED`, not a semantic `DROPPED` state/i);assert.match(evidence,/47ee8e31a08597cffc0c773dfaf72a093501e5c4/);assert.match(evidence,/phase10c-dragged-delivery=PASS count=4/);assert.match(evidence,/emergencyReleasePosted=false/);
});

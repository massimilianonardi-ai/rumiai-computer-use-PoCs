...
test("Phase 10C docs record authoritative public validation and preserve delivery-not-success",()=>{
  const api=fs.readFileSync(path.join(productRoot,"docs/api-pointer.md"),"utf8");const phase10=fs.readFileSync(path.join(productRoot,"docs/phase10-low-level-fallbacks.md"),"utf8");const evidence=fs.readFileSync(path.join(productRoot,"docs/evidence/phase10c-pointer-drag-public-physical.md"),"utf8");
  assert.match(api,/Phase 10C drag: `PHYSICALLY_VALIDATED`/);assert.match(api,/1e0286c271cefddc36be7fc84008083d0658bd82/);assert.match(api,/semanticConsequenceVerified.*false/);
  assert.match(phase10,/Phase 10C drag\/drop\s+PHYSICALLY_VALIDATED/);assert.match(phase10,/working semantic capability always takes precedence/i);
  assert.match(evidence,/mouseDragged\s+count=4/);assert.match(evidence,/no (?:fixture )?emergency release was required/i);assert.match(evidence,/no user content was touched/i);assert.match(evidence,/semanticConsequenceVerified = false|semanticConsequenceVerified.*false/);
});

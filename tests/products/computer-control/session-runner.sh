...
for test_file in "$POC_ROOT"/tests/products/computer-control/contract-tests/*.test.js; do
  run_test "contract:$(basename "$test_file")" "RUMIAI_COMPUTER_CONTROL_ROOT='$PRODUCT_ROOT' '$NODE' --test '$test_file'"
done
run_test "$PHYSICAL_TEST_ID" "AGENT_CTRL='$AGENT_CTRL' RUMIAI_COMPUTER_CONTROL_ROOT='$PRODUCT_ROOT' RUMIAI_CC_NODE='$NODE' '$NODE' '$POC_ROOT/$PHYSICAL_TEST'"
...
fs.writeFileSync(process.env.RESULT_REL,JSON.stringify({sessionId:process.env.SESSION_ID,startedAt:process.env.STARTED_AT,endedAt:process.env.ENDED_AT,durationMs:Number(process.env.TOTAL_MS),productShaExpected:process.env.EXPECTED_PRODUCT_SHA,productShaObserved:process.env.PRODUCT_HEAD,testSourceSha:process.env.TEST_SOURCE_SHA,pocShaTested:process.env.POC_HEAD,tests:rows,summary},null,2)+'\n');
NODE

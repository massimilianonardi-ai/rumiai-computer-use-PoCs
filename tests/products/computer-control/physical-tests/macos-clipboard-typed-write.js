...
function makeQueue(stream){const rl=readline.createInterface({input:stream,crlfDelay:Infinity});const values=[],waiters=[];let ended=false;rl.on("line",line=>{let value;try{value=JSON.parse(line);}catch{value={ok:false,state:"FAILED",error:"GUARDIAN_INVALID_JSON"};}if(waiters.length)waiters.shift().resolve(value);else values.push(value);});rl.on("close",()=>{ended=true;while(waiters.length)waiters.shift().reject(new Error("guardian stdout closed"));});return{next(timeoutMs=10000){if(values.length)return Promise.resolve(values.shift());if(ended)return Promise.reject(new Error("guardian stdout closed"));return new Promise((resolve,reject)=>{const waiter={resolve:value=>{clearTimeout(timer);resolve(value);},reject:error=>{clearTimeout(timer);reject(error);}};const timer=setTimeout(()=>{const index=waiters.indexOf(waiter);if(index>=0)waiters.splice(index,1);reject(new Error("guardian response timeout"));},timeoutMs);waiters.push(waiter);});},close(){rl.close();}};}

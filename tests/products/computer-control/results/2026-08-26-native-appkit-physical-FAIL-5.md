# Native Cocoa/AppKit physical validation — FAIL (run 5)

- Date: 2026-08-26
- Host: physical macOS Apple Silicon
- macOS: 26.5.2 (Build 25F84)
- Product SHA: `114d5a4a757f737b550b5cd36a3eb411f6e745dd`
- PoC SHA before evidence: `3d32f50f4d9fdc812a1cc7372f27ab0c005dbec8`
...
The earlier FAIL evidence files were preserved unchanged. This run uses the `-FAIL-5.md` suffix so that there remains exactly one evidence file for each physical run.
...
Error: Target is not observed inside a native scroll-area
...
The native fixture launched and physically passed checkbox, radio-button, numeric value, outline-row discovery, disclosure, and nested-child assertions. The first remaining failure occurs in `ui.scroll`: the visible anchor is physically inside an `NSScrollView`, but the backend rejects it because the target is not recognized as being inside a native `scroll-area` ancestor.

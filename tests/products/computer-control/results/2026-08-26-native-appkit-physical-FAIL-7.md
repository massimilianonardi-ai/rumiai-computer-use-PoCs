# Native Cocoa/AppKit physical validation — FAIL (run 7)

- Date: 2026-08-26
- Host: physical macOS Apple Silicon
- macOS: 26.5.2 (Build 25F84)
- Product SHA: `56b6052351356ae0c48e1532ee1a7c09dd30a232`
- PoC SHA before evidence: `eab8e6a64ff1e2fa50af912974dcc21ed801d54e`
...
The earlier FAIL evidence files were preserved unchanged. This run uses the `-FAIL-7.md` suffix so that there remains exactly one evidence file for each physical run.
...
The native fixture launched and physically passed every check through `ui.scroll`. The first remaining failure occurs in `ui.scrollIntoView`: the deep AppKit button does not expose the `AXScrollToVisible` action, so `agent-ctrl 0.1.4` returns `kAXErrorAttributeUnsupported`. A verified wheel/geometry fallback is required for this native control.

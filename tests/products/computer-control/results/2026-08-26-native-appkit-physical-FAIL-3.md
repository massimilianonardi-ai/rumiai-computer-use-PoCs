# Native Cocoa/AppKit physical validation — FAIL (run 3)

- Date: 2026-08-26
- Host: physical macOS Apple Silicon
- macOS: 26.5.2 (Build 25F84)
- Product SHA: `b4e68e487bdbd29e323a9ba281f5751b5c4fc1ed`
- PoC SHA before evidence: `1393645544d066a83e523675d8b58f8f852a4056`
...
The earlier FAIL evidence files were preserved unchanged. This run uses the `-FAIL-3.md` suffix so that there remains exactly one evidence file for each physical run.
...
The native fixture launched and physically passed checkbox, radio-button, numeric slider set, increment, and decrement assertions. The first remaining failure occurs immediately afterward when the harness searches for the native `NSOutlineView` row named `RumiAI Native Parent Row`; `ui.find` returns `no match`, so disclosure and children validation cannot start.

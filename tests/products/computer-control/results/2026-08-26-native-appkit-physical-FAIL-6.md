# Native Cocoa/AppKit physical validation — FAIL (run 6)

- Date: 2026-08-26
- Host: physical macOS Apple Silicon
- macOS: 26.5.2 (Build 25F84)
- Product SHA: `7f5c6baee0742ccdee917d017ec2d976c200213f`
- PoC SHA before evidence: `4886e54e977292a3d8e39cfab9c9f80eccb99951`
...
The earlier FAIL evidence files were preserved unchanged. This run uses the `-FAIL-6.md` suffix so that there remains exactly one evidence file for each physical run.
...
Error: Target-aware wheel delivery did not produce an observable change in the native scroll-area subtree
...
The native fixture launched and physically passed all checks through `ui.children`. `ui.scroll` recognized the AppKit scroll group and delivered a target-aware wheel event, but the postcondition detected no Accessibility subtree change. The failure therefore lies in native scroll delivery semantics rather than target/context discovery.

...
const FIXTURE_A = "/tmp/rumiai-v63-window-focus-facade-A.txt";
const FIXTURE_B = "/tmp/rumiai-v63-window-focus-facade-B.txt";
...
async function main() {
  ...
  try {
    fs.writeFileSync(FIXTURE_A, "RumiAI v63 facade focus fixture A\n", "utf8");
    fs.writeFileSync(FIXTURE_B, "RumiAI v63 facade focus fixture B\n", "utf8");
    ...
    console.log(`facade-window-list=${before.ok ? "PASS" : "FAIL"}`);
    console.log(`facade-window-count=${Array.isArray(before.windows) ? before.windows.length : 0}`);
    console.log(`facade-windows=${JSON.stringify(before.windows || [])}`);

    if (!before.ok || !Array.isArray(before.windows)) {
      console.log(`facade-window-list-error=${before.error || before.detail || "window list unavailable"}`);
      failed = true;
      return;
    }
    ...
    const initialPinned = fixtureWindows.find(window => window.pinned === true) || null;
    const target = fixtureWindows.find(window => window.pinned !== true) || null;
    ...
    console.log(`focus-fixture-ready=${validFixtures ? "PASS" : "FAIL"}`);
    if (!validFixtures) {
      failed = true;
      return;
    }
    ...
    console.log(`facade-window-focus=${focused.ok ? "PASS" : "FAIL"}`);
    console.log(`facade-window-focus-state=${focused.state || ""}`);
    console.log(`facade-window-focus-method=${focused.method || ""}`);
    console.log(`facade-window-focus-verified=${focused.verified === true}`);
    console.log(`facade-window-focus-verification=${focused.verificationMethod || ""}`);
    console.log(`facade-focused-window=${JSON.stringify(focused.window || null)}`);

    const facadeVerified = Boolean(
      focused.ok &&
    ...
    console.log(`facade-focus-postcondition=${facadeVerified ? "PASS" : "FAIL"}`);

    if (!facadeVerified) {
      console.log(`facade-window-focus-error=${focused.error || focused.detail || "focus unverified"}`);
      failed = true;
      return;
    }

    const independent = await waitForFrontDocument(target.title);
    ...
    if (!independent.ok) {
      console.log(`independent-focus-error=${independent.detail || "front document did not become target"}`);
      failed = true;
      return;
    }

    console.log("physical-window-focus-facade=PASS");
  } finally {
...
main().catch(error => {
  console.error("physical-window-focus-facade=FAIL");
